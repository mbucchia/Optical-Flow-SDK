...
typedef struct CUDA_MEM_ALLOC_NODE_PARAMS_st {
    CUmemPoolProps poolProps; /**< in: requested memory type and placement */
    const CUmemAccessDesc *accessDescs; /**< in: peer access descriptor for memory node */
    size_t accessDescCount; /**< in: Number of `accessDescs`s */
    size_t bytesize; /**< in: size in bytes */
    CUdeviceptr dptr; /**< out: address returned by CUDA */
} CUDA_MEM_ALLOC_NODE_PARAMS;

typedef enum CUgraphMem_attribute_enum {
    /**
     * (value type = cuuint64_t)
     * Amount of memory currently in bytes associated with graphs
     */
     ...
     * cuda graph's asynchronous allocator.
     */
 ...
 *
 * Creates a new memory allocation node and adds it to \p hGraph with \p numDependencies
 * dependencies specified via \p dependencies and arguments specified in \p nodeParams.
 * It is possible for \p numDependencies to be 0, in which case the node will be placed
 * at the root of the graph. \p dependencies may not have any duplicate entries. A handle
 * to the new node will be returned in \p phGraphNode.
 ...
 *
 * Supported attributes: CU_GRAPH_MEM_ATTR_USED_MEM_CURRENT, CU_GRAPH_MEM_ATTR_USED_MEM_HIGH, CU_GRAPH_MEM_ATTR_RESERVED_MEM_CURRENT, CU_GRAPH_MEM_ATTR_RESERVED_MEM_HIGH.
 * // TODO: add detailed descriptions of the attributes here.
 *
 ...
 *
 * Supported attributes: CU_GRAPH_MEM_ATTR_USED_MEM_HIGH, CU_GRAPH_MEM_ATTR_RESERVED_MEM_HIGH.
 * // TODO: add detailed descriptions of the attributes here.
 *
 * \param device - Specifies the scope of the query
 * \param attr - attribute to get
